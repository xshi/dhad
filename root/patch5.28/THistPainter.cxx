...
void THistPainter::PaintAxis(Bool_t drawGridOnly)
{
   ...
   TGaxis axis;
   TF1* transform = NULL;
   TF1 loc_sqrt("sqrt", "TMath::Sqrt(x)", 0, 1);
   ...
   } else if (Hoption.Logx == 2) {
     //      transform = dynamic_cast<TF1*>(gROOT->GetFunction("pol2")->Clone());
     transform = dynamic_cast<TF1*>(loc_sqrt.Clone());
     transform->SetName("pol2x_histpainter_transform");
     // set constant, linear term zero
     //      transform->SetParameter(0,0); transform->SetParameter(1,0);
     ndiv = TMath::Abs(ndivx);
     if (useHparam) {
       umin = Hparam.xmin;
         umax = Hparam.xmax;
   ...
   // if transform is non-NULL, set transform, make transform NULL
   if (transform) {
     axis.SetFunction("pol2x_histpainter_transform");
     transform = NULL;
   }
   ...
   } else if (Hoption.Logy == 2) { 
//      transform = dynamic_cast<TF1*>(gROOT->GetFunction("pol2")->Clone());
      transform = dynamic_cast<TF1*>(loc_sqrt.Clone());
      transform->SetName("pol2y_histpainter_transform");
      // set constant, linear term zero
//      transform->SetParameter(0,0); transform->SetParameter(1,0);
      ndiv = TMath::Abs(ndivy);
      if (useHparam) {
         umin = Hparam.ymin*Hparam.ymin;
         umax = Hparam.ymax*Hparam.ymax;
      } else {
         umin = aymin*aymin;
         umax = aymax*aymax;
      }
...
//       }
   ...
   // if transform is non-NULL, set transform
   if (transform) {
      axis.SetFunction("pol2y_histpainter_transform");
      transform = NULL;
   }
...
void THistPainter::PaintBoxes(Option_t *)
{
   ...
   } else if (Hoption.Logz == 2) {
      if (zmin >= 0) {
         zmin = TMath::Sqrt(zmin*0.1);
         zmax = TMath::Sqrt(zmax);
      } else {
         return;
      }
   } else {
   ...
   if (Hoption.Same) {
	 ...
	 if (Hoption.Logz == 1) {
            ...
            zmax = TMath::Sqrt(zmax);
            if (zmin < 0) {
               zmin = TMath::Sqrt(zmax*0.001);
            } else {
               zmin = TMath::Sqrt(zmin);
            }
         }
         break;
      }
...
*/
   ...
   for (Int_t j=Hparam.yfirst; j<=Hparam.ylast;j++) {
      ...
      for (Int_t i=Hparam.xfirst; i<=Hparam.xlast;i++) {
	   ...
	   z = TMath::Sqrt(z); // supposedly z < 0 removed so no extra check
	 }
	 ...
	 if (Hoption.Logx == 1) {
            ...
            if (xup >= 0)  xup  = TMath::Sqrt(xup);
            else continue;
            if (xlow >= 0) xlow = TMath::Sqrt(xlow);
            else continue;
         }
	 ...
	 if (Hoption.Logy == 1) {
            ...
            if (yup >= 0)  yup  = TMath::Sqrt(yup);
            else continue;
            if (ylow >= 0) ylow = TMath::Sqrt(ylow);
            else continue;
	 }
...
void THistPainter::PaintColorLevels(Option_t *)
{
   ...
   for (Int_t j=Hparam.yfirst; j<=Hparam.ylast;j++) {
      ...
      for (Int_t i=Hparam.xfirst; i<=Hparam.xlast;i++) {
         ...
         //if (Hoption.Logz) {
	 if (z == 0 && (zmin >= 0 || Hoption.Logz == 1)) continue; // don't draw the empty bins for histograms with positive content
         if (Hoption.Logz == 1) {
	   if (z > 0) z = TMath::Log10(z);
	   else       z = zmin;
         } else if (Hoption.Logz == 2) {
            if (z >= 0) z = TMath::Sqrt(z);
            else       z = zmin;
         }
         if (z < zmin) continue;
         xup  = xk + xstep;
...
void THistPainter::PaintContour(Option_t *option)
{
   ...
   for (j=Hparam.yfirst; j<Hparam.ylast; j++) {
	 ...
	 if (Hoption.Logz == 1) {
            ...
            if (zc[0] >= 0)   zc[0] = TMath::Sqrt(zc[0]);
            else             zc[0] = Hparam.zmin;
            if (zc[1] >= 0)   zc[1] = TMath::Sqrt(zc[1]);
            else             zc[1] = Hparam.zmin;
            if (zc[2] >= 0)   zc[2] = TMath::Sqrt(zc[2]);
            else             zc[2] = Hparam.zmin;
            if (zc[3] >= 0)   zc[3] = TMath::Sqrt(zc[3]);
            else             zc[3] = Hparam.zmin;
         }
         for (k=0;k<4;k++) {
...
void THistPainter::PaintErrors(Option_t *)
{
   ...
   if (fixbin) {
     ...
     if (Hoption.Logx == 1) xp = TMath::Power(10,Hparam.xmin) + 0.5*Hparam.xbinsize;
     else if (Hoption.Logx == 2) xp = Hparam.xmin*Hparam.xmin + 0.5*Hparam.xbinsize;
   ...
   if (Hoption.Logx == 1) logxmin = TMath::Power(10,Hparam.xmin);
   else if (Hoption.Logx == 2) logxmin = Hparam.xmin*Hparam.xmin;
   if (Hoption.Logy == 1) logymin = TMath::Power(10,Hparam.ymin);
   else if (Hoption.Logy == 2) logymin = Hparam.ymin*Hparam.ymin;
   ...
   for (k=first; k<=last; k++) {
      ...
      if (Hoption.Logx == 1) {
         ...
         xi1 = TMath::Sqrt(TMath::Max(xi1,logxmin));
         xi2 = TMath::Sqrt(TMath::Max(xi2,logxmin));
         xi3 = TMath::Sqrt(TMath::Max(xi3,logxmin));
         xi4 = TMath::Sqrt(TMath::Max(xi4,logxmin));
      }
      //if (Hoption.Logy) {
      if (Hoption.Logy == 1) {
         ...
         yi1 = TMath::Sqrt(TMath::Max(yi1,logymin));
         yi2 = TMath::Sqrt(TMath::Max(yi2,logymin));
         yi3 = TMath::Sqrt(TMath::Max(yi3,logymin));
         yi4 = TMath::Sqrt(TMath::Max(yi4,logymin));
      }
...
void THistPainter::Paint2DErrors(Option_t *)
{
   ...
   for (Int_t j=Hparam.yfirst; j<=Hparam.ylast;j++) {
	...
	if (y > 0)  y = TMath::Log10(y);
         ...
         if (y >= 0)  y = TMath::Sqrt(y);
         else         continue;
         if (y1 >= 0) y1 = TMath::Sqrt(y1);
         else         y1 = Hparam.ymin;
         if (y2 >= 0) y2 = TMath::Sqrt(y2);
         else         y2 = Hparam.ymin;
      }
      yk    = fYaxis->GetBinLowEdge(j);
      ystep = fYaxis->GetBinWidth(j);
	 ...
	 if (Hoption.Logx == 1) {
            ...
            if (x >= 0)  x = TMath::Sqrt(x);
            else         continue;
            if (x1 >= 0) x1 = TMath::Sqrt(x1);
            else         x1 = Hparam.xmin;
            if (x2 >= 0) x2 = TMath::Sqrt(x2);
            else         x2 = Hparam.xmin;
         }
         z  = fH->GetBinContent(bin);
	 ...
	 if (Hoption.Logz == 1) {
            ...
            if (z1 >= 0) z1 = TMath::Sqrt(z1);
            else         z1 = Hparam.zmin;
            if (z2 >= 0) z2 = TMath::Sqrt(z2);
            else         z2 = Hparam.zmin;
         }
...
void THistPainter::PaintHist(Option_t *)
{
   ...
   if (Hoption.Logy == 1) logymin = TMath::Power(10,ymin);
   else if (Hoption.Logy == 2) logymin = ymin*ymin;
   ...
   for (j=first; j<=last;j++) {
	...
	if (Hoption.Logy == 1) yb = TMath::Log10(TMath::Max(c1,.1*logymin));
	else if (Hoption.Logy == 2) yb = TMath::Sqrt(TMath::Max(c1,.1*logymin));
	else              yb = c1;
      }
...
Int_t THistPainter::PaintInit()
{
   ...
   if (Hoption.Logy == 1 && xm <= 0) {
      Error(where, "log scale requested with a negative argument (%f)", xm);
      return 0;
   } else if (Hoption.Logy == 2 && xm < 0) {
      Error(where, "sqrt scale requested with negative argument (%f)", xm);
      return 0;
   ...
   if (ymin >= ymax) {
	 ...
	 //      else {
         ...
         if (ymax >= 0) ymin = 0.001*ymax;
            ...
            if (!Hoption.Same) Error(where, "sqrt scale is requested but maximum is less than 0 (%f)", ymax);
            return 0;
         }
      } else {
         if (ymin > 0) {
   ...
   if (Hoption.Logy == 1) {
      ...
      if (ymin <0 || ymax <0) {
         Error(where, "Cannot set Y axis to sqrt scale");
         return 0;
      }
      ymin = TMath::Sqrt(ymin);
//      if (!minimum) ymin = 0;
      ymax = TMath::Sqrt(ymax);
//      if (!maximum && !Hoption.Plus) ymax += TMath::Sqrt(2*(0.9/0.95));
      if (!Hoption.Same) {
         Hparam.ymin = ymin;
         Hparam.ymax = ymax;
      }
...
Int_t THistPainter::PaintInitH()
{
   ...
   if (Hoption.Logy == 1) {
      ...
      Hparam.xfirst= fXaxis->FindFixBin(Hparam.ymin);
      Hparam.xlast = fXaxis->FindFixBin(Hparam.ymax);
      Hparam.ymin  = TMath::Sqrt(Hparam.ymin);
      Hparam.ymax  = TMath::Sqrt(Hparam.ymax);
      if (Hparam.xlast > last) Hparam.xlast = last;
   }
     ...
     if (Hoption.Logx == 1) {
      ...
      // else {
         ...
         if (xmax >= 0) xmin = 0.001*xmax;
            ...
            if (!Hoption.Same) Error(where, "log scale is requested but maximum is less than 0 (%f)", xmax);
            return 0;
         }
      } else {
         if (xmin > 0) {
   ...
   if (Hoption.Logx == 1) {
      ...
      xmin = TMath::Sqrt(xmin);
      if (!minimum) xmin += TMath::Sqrt(0.5);
      xmax = TMath::Sqrt(xmax);
      if (!maximum) xmax += TMath::Sqrt(2*(0.9/0.95));
      if (!Hoption.Same) {
         Hparam.xmin = xmin;
         Hparam.xmax = xmax;
      }
...
void THistPainter::PaintLego(Option_t *)
{
   ...
   if (Hoption.System == kPOLAR) {
     ...
     if (Hoption.Logy == 1) {
         ...
         if (ylab1 >= 0) fXbuf[2] = TMath::Sqrt(ylab1);
         else            fXbuf[2] = 0;
         if (ylab2 >= 0) fYbuf[2] = TMath::Sqrt(ylab2);
         else            fYbuf[2] = 0;
      } else {
         fXbuf[2] = ylab1;
         fYbuf[2] = ylab2;
...
void THistPainter::PaintLegoAxis(TGaxis *axis, Double_t ang)
{
   ...
   if (TMath::Abs(x1[0] - x2[0]) >= epsil || TMath::Abs(x1[1] - x2[1]) > epsil) {
      ...
      if ((Hoption.Logx == 1) && !fH->InheritsFrom(TH3::Class())) {
         bmin = TMath::Power(10, rmin[0]);
         bmax = TMath::Power(10, rmax[0]);
      } else if ((Hoption.Logx == 2) && !fH->InheritsFrom(TH3::Class())) {
         bmin = rmin[0]*rmin[0];
         bmax = rmax[0]*rmax[0];
      } else {
         bmin = rmin[0];
   ...
   if (TMath::Abs(y1[0] - y2[0]) >= epsil || TMath::Abs(y1[1] - y2[1]) > epsil) {
      ...
      if ((Hoption.Logy == 1) && !fH->InheritsFrom(TH3::Class())) {
         bmin = TMath::Power(10, rmin[1]);
         bmax = TMath::Power(10, rmax[1]);
      } else if ((Hoption.Logy == 2) && !fH->InheritsFrom(TH3::Class())) {
	bmin = rmin[1]*rmin[1];
	bmax = rmax[1]*rmax[1];
      } else {
         bmin = rmin[1];
   ...
   if (TMath::Abs(z1[0] - z2[0]) >= 100*epsil || TMath::Abs(z1[1] - z2[1]) > 100*epsil) {
      ...
      if ((Hoption.Logz == 1) && !fH->InheritsFrom(TH3::Class())) {
         bmin = TMath::Power(10, rmin[2]);
         bmax = TMath::Power(10, rmax[2]);
      } else if ((Hoption.Logz == 2) && !fH->InheritsFrom(TH3::Class())) {
	bmin = rmin[2]*rmin[2];
	bmax = rmax[2]*rmax[2];
      } else {
         bmin = rmin[2];
...
void THistPainter::PaintScatterPlot(Option_t *option)
{
   ...
   if (Hoption.Logz == 1) {
      ...
      if (zmin >= 0) zmin = TMath::Sqrt(zmin);
      else           zmin = 0;
      if (zmax >= 0) zmax = TMath::Sqrt(zmax);
      else           zmax = 0;
      if (zmin == 0 && zmax == 0) return;
      dz = zmax - zmin;
      scale = 100/dz;
	   ...
	   z    -=  zmin;
         ...
         if (k > 0) {
	       ...
	       if (Hoption.Logx == 1){
                  ...
                  if (fXbuf[marker] > 0) fXbuf[marker] = TMath::Sqrt(fXbuf[marker]);
                  else                   break;
               }
               //if (Hoption.Logy){
	       if (Hoption.Logy == 1){
                  ...
                  if (fYbuf[marker] > 0) fYbuf[marker] = TMath::Sqrt(fYbuf[marker]);
                  else                  break;
               }
...
Int_t THistPainter::TableInit()
{
   ...
   //   find the first edge of a bin that is >= 0
      ...
      if (Hparam.xmin <0 || Hparam.xmax <0) {
         Error(where, "cannot set X axis to sqrt scale");
         return 0;
      }
      Hparam.xfirst= fXaxis->FindFixBin(Hparam.xmin);
      if (Hparam.xfirst < first) Hparam.xfirst = first;
      Hparam.xlast = fXaxis->FindFixBin(Hparam.xmax);
      if (Hparam.xlast > last) Hparam.xlast = last;
      Hparam.xmin  = TMath::Sqrt(Hparam.xmin);
      Hparam.xmax  = TMath::Sqrt(Hparam.xmax);
   }
   ...
   if (Hoption.Logy == 1) {
      ...
      Hparam.ylast = fYaxis->FindFixBin(Hparam.ymax);
      if (Hparam.ylast > last) Hparam.ylast = last;
      Hparam.ymin  = Hparam.ymin;
      Hparam.ymax  = Hparam.ymax;
   }
   ...
   if (Hoption.Logz == 1 && zmax <= 0) {
      if (!Hoption.Same) Error(where, "log scale is requested but maximum is less or equal 0 (%f)", zmax);
      return 0;
   } else if (Hoption.Logz == 1 && zmin>=0 && zmax==0) { // empty histogram in log scale
      zmin = 0.01;
      zmax = 10.;
   } else if (Hoption.Logz == 2 && zmax < 0) {
      if (!Hoption.Same) Error(where, "sqrt scale is requested but maximum is less than 0 (%f)", zmax);
      return 0;
   }
   if (zmin >= zmax) {
     //if (Hoption.Logz) {
   ...
   if (Hoption.Logz) {
     ...
     if (Hoption.Logz == 1) {
      ...
      zmin = TMath::Sqrt(zmin);
      if (!minimum) zmin += TMath::Sqrt(0.5);
      zmax = TMath::Sqrt(zmax);
      if (!maximum) zmax += TMath::Sqrt(2*(0.9/0.95));
      goto LZMIN;
   }
