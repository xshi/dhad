...
void TGraphPainter::ComputeLogs(Int_t npoints, Int_t opt)
{
   ...
   } else if (gPad->GetLogx() == 2) {
     for (i=0;i<npoints;i++) {
       if (gxworkl[i] >= 0) gxworkl[i] = TMath::Sqrt(gxworkl[i]);
       else                gxworkl[i] = gPad->GetX1();
     }
   }
   //if (!opt && gPad->GetLogy()) {
   if (!opt && gPad->GetLogy() == 1) {
      for (i=0;i<npoints;i++) {
         if (gyworkl[i] > 0) gyworkl[i] = TMath::Log10(gyworkl[i]);
         else                gyworkl[i] = gPad->GetY1();
      }
   } else if (!opt && gPad->GetLogy() == 2) {
     for (i=0;i<npoints;i++) {
       if (gyworkl[i] > 0) gyworkl[i] = TMath::Sqrt(gyworkl[i]);
       else                gyworkl[i] = gPad->GetY1();
     }
   }
}
                                  ...
                                  const Double_t *y, Option_t *chopt)
{
   ...
   if (optionAxis) {
	...
	transform->SetName("pol2_histpainter_transform");
	...
	transform->SetParameter(0,0); transform->SetParameter(1,0);
      ...
      if (transform)
	axis->SetFunction("pol2_histpainter_transform");
	...
	TF1 loc_sqrt("sqrt", "TMath::Sqrt(x)", 0, 1);
	 ...
	 //	 transform->SetParameter(0,0); transform->SetParameter(1,0);
      ...
      // if transform is non-NULL, set transform
      if (transform) {
	axis->SetFunction("pol2y_graph_transform");
	transform = NULL;
      }
   ...
   if ((optionMarker) || (optionLine)) {
      ...
      if (lowRes) {
         ...
         for (ipix = 0; ipix < nrPix; ipix++) {
	       ...
	       } else if (!optionZ && gPad->GetLogx() == 2) {
                 if (xc[nrLine] >= 0) xc[nrLine] = TMath::Sqrt(xc[nrLine]);
                 else                 xc[nrLine] = gPad->GetX1();
                 for (Int_t il = 0; il < 2; il++) {
		   if (xl[il] >= 0) xl[il] = TMath::Sqrt(xl[il]);
		   else             xl[il] = gPad->GetX1();
                 }
	       }
               ...
               } else if (!optionZ && gPad->GetLogy() == 2) {
		 if (yc[nrLine] >= 0) yc[nrLine] = TMath::Sqrt(yc[nrLine]);
		 else                 yc[nrLine] = gPad->GetY1();
		 for (Int_t il = 0; il < 2; il++) {
		   if (yl[il] >= 0) yl[il] = TMath::Sqrt(yl[il]);
		   else             yl[il] = gPad->GetY1();
		 }
	       }
