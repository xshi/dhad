...
Double_t TPad::PadtoX(Double_t x) const
{
  ...
  if (fLogx == 1 && x < 50) return Double_t(TMath::Exp(2.302585092994*x));
  if (fLogx == 2) return Double_t(x*x);
  return x;
}
...
Double_t TPad::PadtoY(Double_t y) const
{
   ...
   if (fLogy == 1 && y < 50) return Double_t(TMath::Exp(2.302585092994*y));
   if (fLogy == 2) return Double_t(y*y);
   return y;
}
