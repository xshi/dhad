...
from tools.fits import mbc_single_3s
...
from fit import kkmass

kkmass.fit_single_mode("%s", "%s", "%s", %s, %s, %s)
...
'''% (datatype, mode, label, lowmass, highmass, opts.test)
    ...
    file_ = os.path.join(attr.datpath, datatype, label, 'src',
                         'kkmass/%s_%s' % (lowmass, highmass) , filename)
