...
sys.stdout.write('Loading syserr_818ipbv12.7 ...')
## ----------------------------------------------------------
##  Systematic  Errors
## ----------------------------------------------------------
...
sys_err = {
    ...
    'Fractional uncertainty per single tag associated with fit parameter':{
        'ND0D0Bar'    : [0] ,
        'BrD2KPi'     : [ 0, #float(tab.cell_get('Tracking', 'Kpi')), 
                          float(tab.cell_get('Signal shape', 'Kpi')) ],

        'BrD2KPiPi0'  : [ 0, #float(tab.cell_get('Tracking', 'Kpipi0')), 
                          float(tab.cell_get('Signal shape', 'Kpipi0')) ],
        
        'BrD2KPiPiPi' :  [ 0, #float(tab.cell_get('Tracking', 'K3pi')), 
                          float(tab.cell_get('Signal shape', 'K3pi')) ],

        'ND+D-'       : [0] ,

        'BrD2KPiPi'   :  [ 0, #float(tab.cell_get('Tracking', 'Kpipi')), 
                          float(tab.cell_get('Signal shape', 'Kpipi')) ],

        'BrD2KPiPiPi0':  [ 0, #float(tab.cell_get('Tracking', 'Kpipipi0')), 
                          float(tab.cell_get('Signal shape', 'Kpipipi0')) ],

        'BrD2KsPi'    :  [ 0, #float(tab.cell_get('Tracking', 'Kspi')), 
                          float(tab.cell_get('Signal shape', 'Kspi')) ],

        'BrD2KsPiPi0' :  [ 0, #float(tab.cell_get('Tracking', 'Kspipi0')), 
                          float(tab.cell_get('Signal shape', 'Kspipi0')) ],

        'BrD2KsPiPiPi':  [ 0, #float(tab.cell_get('Tracking', 'Ks3pi')), 
                          float(tab.cell_get('Signal shape', 'Ks3pi')) ],

        'BrD2KKPi'    :  [ 0, #float(tab.cell_get('Tracking', 'KKpi')), 
