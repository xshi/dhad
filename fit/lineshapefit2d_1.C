...
void lineshapefit2d_1(TString mode1,
		    double dsigma1,
		    double df1a,
		    double df1b,
		    double ds1a,
  ...
  TCanvas *canvas=new TCanvas("c","DLineShape",600,400);
...
*/
  ...
  mbcFrame->GetXaxis()->SetTitle("M_{BC} (GeV/c^{2})");
  mbcFrame->GetYaxis()->SetTitle("Events / (1 MeV/c^{2})");
  mbcFrame->GetYaxis()->SetTitleOffset(1.5);
  ...
  totalPdf.plotOn(mbcFrame,Components(RooArgSet(BkgdFlat,Bkgd1,Bkgd2,Bkgd3)),LineColor(kBlue), LineStyle(kDashed));
  mbcFrame->Draw();
